...
local user_opts = {
    ...
    window_controls_size = 16,             -- 右上角窗口控制按钮和 Logo 的尺寸大小
...
local function set_osc_styles()
    ...
    osc_styles = {
        ...
        window_control = "{\\1c&H" .. osc_color_convert(user_opts.window_controls_color) .. "&\\fs" .. user_opts.window_controls_size .. "\\fn" .. icons.iconfont .. "}",
        window_title = "{\\bord0\\shad1.5\\1c&H" .. osc_color_convert(user_opts.window_title_color) .. "&\\3c&H000000&\\3a&HAA&\\fs".. user_opts.window_title_font_size .."\\q2\\fn" .. user_opts.font .. "}",
...
local function prepare_elements()
    ...
    for _,element in pairs(elements) do
        ...
        -- Special gray out logic for audio and subtitle track buttons
        if element.name == "audio_track" then
            local count = state.audio_track_count or 0
            if count == 0 then
                element.layout.alpha[1] = 200  -- 不存在轨道：完全置灰 (第二档置灰)
            elseif element.off then
                element.layout.alpha[1] = 150  -- 存在但关闭：第一档置灰 (与“下一个视频”一致)
            else
                element.layout.alpha[1] = 0    -- 存在且开启：正常显示
            end
        elseif element.name == "sub_track" then
            local count = state.sub_track_count or 0
            if count == 0 then
                element.layout.alpha[1] = 200  -- 不存在轨道：完全置灰 (第二档置灰)
            elseif element.off then
                element.layout.alpha[1] = 150  -- 存在但关闭：第一档置灰 (与“下一个视频”一致)
...
local function render_elements(master_ass, osc_vis, wc_vis)
    local function render_element(n)
        ...
        if element.eventresponder and (state.active_element == n) then
            ...
            if mouse_hit(element) then
                ...
                if element.styledown then
                    ...
                    if element.name == "title" or element.name == "chapter_title" or element.name == "time_codes" or element.name == "duration_code" or element.name == "speed" or element.name == "cache_info" then
        ...
        if element.type == "button" and hover_effects.box
            ...
            and element.name ~= "duration_code"
        ...
        elseif element.type == "button" then
            ...
            if hovered and (hover_effects.size or hover_effects.color) then
                ...
                if element.name == "title" or element.name == "time_codes" or element.name == "duration_code" or element.name == "chapter_title" or element.name == "cache_info" or element.name == "speed" then
            ...
            if (element.tooltipF ~= nil or element.nothingavailable ~= nil) and not seeking_with_force_tooltip and user_opts.tooltip_hints then
                local hb = element.hover_box
                if (hb and mouse_hit_coords(hb.x1, hb.y1, hb.x2, hb.y2) or mouse_hit(element)) then
                    ...
                    if element.enabled then
                        ...
                        tooltiplabel = type(element.nothingavailable) == "function" and element.nothingavailable() or element.nothingavailable
                    end
...
local function window_controls()
    local btn_width = math.floor(user_opts.window_controls_size * 2.5)
    local wc_geo = {
        x = 0,
        y = btn_width,
        an = 1,
    ...
    local controlbox_w = (user_opts.window_controls and (btn_width * 3) or 0)
    local controlbox_left = wc_geo.w - controlbox_w
    local titlebox_left = ontop_active and btn_width or wc_geo.x
    local button_y = wc_geo.y - (wc_geo.h / 2)
    local first_geo  = {x = controlbox_left + btn_width * 0.5,  y = button_y, an = 5, w = btn_width, h = wc_geo.h}
    local second_geo = {x = controlbox_left + btn_width * 1.5, y = button_y, an = 5, w = btn_width, h = wc_geo.h}
    local third_geo  = {x = controlbox_left + btn_width * 2.5, y = button_y, an = 5, w = btn_width, h = wc_geo.h}
    ...
    if ontop_active then
        ...
        lo = add_layout("ontop")
        lo.geometry = {x = btn_width * 0.5, y = button_y, an = 5, w = btn_width, h = wc_geo.h}
        lo.style = osc_styles.window_control
        lo.group = "top"
    ...
    if user_opts.show_window_title then
        local title_x = titlebox_left + 15
        -- 使用 an = 1 (左下对齐)，y 坐标利用公式 (wc_geo.h + font_size) / 2 动态计算以确保居中
        local title_y = (wc_geo.h + user_opts.window_title_font_size) / 2
        lo = add_layout("windowtitle")
        lo.geometry = {x = title_x, y = title_y, an = 1, w = controlbox_left - title_x, h = wc_geo.h}
        lo.group = "top"
        lo.alpha[3] = 0
        ...
        state.windowtitle_max_w = controlbox_left - title_x
    end
    ...
    if (user_opts.show_window_title or user_opts.window_controls or ontop_active) then
        ...
        if ontop_active then add_area("window-controls-ontop", 0, 0, btn_width, wc_geo.h) end
...
layouts["default"] = function ()
    ...
    -- seekbar
    -- 计算单个时间的宽度，并在播放中保持宽度固定以防进度条抖动
    local function get_single_time_width()
        local dur = state.duration or 0
        local has_h = (dur >= 3600) or user_opts.time_format == "fixed"
        local base = has_h and "88:88:88" or "88:88"
        local text = base .. (state.tc_ms and ".888" or "")
        return estimate_text_width(text, osc_styles.time)
    end

    local single_time_w = get_single_time_width()
    local left_padding = 15 + single_time_w + 10
    local right_padding = 15 + single_time_w + 10

    ne = new_element("seekbarbg", "box")
    ne.visible = user_opts.nibbles_style ~= "gap"
    lo = add_layout("seekbarbg")
    local seekbar_bg_h = seekbar_height_style.height
    lo.geometry = {x = left_padding, y = refY - user_opts.osc_height, an = 4, w = osc_geo.w - left_padding - right_padding, h = seekbar_bg_h}
    lo.layer = 15
    ...
    local seekbar_h = 18
    lo.geometry = {x = left_padding, y = refY - user_opts.osc_height, an = 4, w = osc_geo.w - left_padding - right_padding, h = seekbar_h}
    lo.layer = 49
    ...
    -- 将当前播放时间与总时长定位在进度条的两端水平线上
    elements["time_codes"].visible = (state.duration or 0) > 0
    lo = add_layout("time_codes")
    lo.geometry = {x = 15, y = refY - user_opts.osc_height, an = 4, w = single_time_w, h = user_opts.time_font_size}
    lo.layer = 48
    lo.alpha[3] = 0
    lo.style = osc_styles.time

    elements["duration_code"].visible = (state.duration or 0) > 0
    lo = add_layout("duration_code")
    lo.geometry = {x = osc_geo.w - 15, y = refY - user_opts.osc_height, an = 6, w = single_time_w, h = user_opts.time_font_size}
    lo.layer = 48
    lo.alpha[3] = 0
    lo.style = osc_styles.time
    ...
    if user_opts.show_chapter_title then
        elements["chapter_title"].visible = false -- 已移动到滑块气泡跟随移动，在此处不再渲染
    ...
    if user_opts.audio_tracks_button then left_side_button("audio_track", 650) end
    if user_opts.subtitles_button then left_side_button("sub_track", 750) end
    ...
    if audio_track and user_opts.volume_control then
        ...
        lo.geometry = {x = start_x, y = refY - (user_opts.osc_height / 2), an = 4, w = 85, h = 4}
        lo.layer = 15
        lo.alpha[1] = 128
        ...
        lo.geometry = {x = start_x, y = refY - (user_opts.osc_height / 2), an = 4, w = 85, h = 10}
        ...
        lo.slider.tooltip_an = 2
        start_x = start_x + 105
        -- vc_center = start_x - 125
        local vc_left = start_x - 137
        local osc_mid = refY - (user_opts.osc_height / 2)
        elements["vol_ctrl"].hover_box = vol_vis and {x1 = vc_left, y1 = osc_mid - 12, x2 = vc_left + 117, y2 = osc_mid + 12} or nil
    end
...
local function format_time(seconds)
    ...
    local show_hours = hours > 0 or user_opts.time_format == "fixed" or duration >= 3600
    ...
    if state.tc_ms then
        ...
        if show_hours then
            ...
            local fmt = (duration >= 600) and "%02d:%02d.%03d" or "%d:%02d.%03d"
        ...
        if show_hours then
            ...
            local fmt = (duration >= 600) and "%02d:%02d" or "%d:%02d"
...
local function osc_init()
    ...
    ne.enabled = state.audio_track_count > 1
    ...
    ne.nothingavailable = function () return state.audio_track_count == 1 and track_tooltip(locale.audio, "audio", "aid", state.audio_track_count) or locale.no_audio end
    bind_buttons("audio_track")

    --sub_track
    ne = new_element("sub_track", "button")
    ne.enabled = state.sub_track_count > 1
    ...
    ne.nothingavailable = function () return state.sub_track_count == 1 and track_tooltip(locale.subtitle, "sub", "sid", state.sub_track_count) or locale.no_subs end
    ...
    ne.content = function()
        ...
        return prefix .. format_time(playtime_remaining)
    end
    ne.eventresponder["mbtn_left_up"] = function()
    ...
    -- Duration display
    ne = new_element("duration_code", "button")
    ne.content = function()
        if not state.duration then return "--:--" end
        return format_time(state.duration)
    end
...
local function validate_user_opts()
    ...
    if user_opts.window_controls_size < 4 then
        msg.warn("window_controls_size must be 4 or higher. Setting it to 4 (minimum).")
        user_opts.window_controls_size = 4
    end
