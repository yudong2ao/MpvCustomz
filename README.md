# MPV ModernZ Optimized 极简高画质配置包

这是一个为 MPV 播放器打造的**开箱即用、高画质沉浸式无边框**配置包。基于现代化界面 `ModernZ` 与进度条预览 `thumbfast` 深度调优。
...
相比 ModernZ 原版，本配置包进行了以下精简实用的细节优化：
...
1. **解压覆盖**：下载打包好的 `.zip` 文件，解压并将所有文件复制覆盖到您的 MPV 安装目录下。
2. **一键注册关联**：
